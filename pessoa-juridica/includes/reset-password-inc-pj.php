<?php
  ...
  if (isset($_POST["reset-password-submit"])) {
    ...
    $query = "SELECT * FROM usuariosPJ WHERE email='$email'";
    $run = mysqli_query($conn, $query);
    if(mysqli_num_rows($run)>0){
      $selector = bin2hex(random_bytes(8));
      $token = random_bytes(32);

      $url = "https://www.hagile.com.br/projetos/teste-sistema-matheus/complexo-seguro/pessoa-juridica/create-new-password-pj.php?selector=" . $selector . "&validator=" . bin2hex($token);
      ...
      $sql = "INSERT INTO pwdResetPJ (pwdResetEmail, pwdResetSelector, pwdResetToken, pwdResetExpires) VALUES (?, ?, ?, ?);";
      $stmt = mysqli_stmt_init($conn);
      if (!mysqli_stmt_prepare($stmt, $sql)) {
        exit();
      } else {
      ...
      header("Location: ../reset-password-pj.php?reset=success");

    }else {
      header("Location: ../reset-password-pj.php?reset=fail");
    }
  }else {
